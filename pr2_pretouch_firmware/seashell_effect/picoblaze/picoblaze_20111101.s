...
init:
	...
	; Use timerB
	; Setup timerB to expire every 125us (8,000Hz)
	; load s0, 7d ; 0.001ms * 125 = 0.125ms   125 = 0x7d
	; Setup timerB to expire every 63us (16,000Hz)
	 load s0, 3f ; 0.001ms * 100 = 0.100ms   100 = 0x64
	output s0, TIMER_B_COMPARE_REG
	load s0, 0  
	output s0, TIMER_B_COUNTER_REG   ; reset Timer B counter to 0
	load s0, TIMER_OVERFLOW_FLAG
	output s0, TIMER_B_OVERFLOW_REG  ; clear overflow flag
...
main_loop:
	...
	; How many times we want to read the sensors for each buffer?
	load s1, 58 ; 88(0x58) * 1 channel
